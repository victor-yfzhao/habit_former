...
public class MessageService {
    //日程安排prompt模板
    //不能有换行符！不然会报很奇怪的错
    private static final String PLANNER_PROMPT="能力与角色:我希望你是一位熟悉{theme}领域的专家 " +
    ...
    private static final String PLANNER_PROMPT_STUDY="能力与角色:我希望你是一位熟悉学习教育领域的专家 " +
            ...
            "输出风格:我希望你能够将输出封装为一个JSON格式的数组，数组中的每个元素代表一个任务项。每个任务项包含如下属性，表示第几天的int型属性'day',string型属性任务名'task_name',string型属性具体任务内容'task_content',string型属性任务涉及的科目名称'studySubject'，（如英语、数学等广义的学科名称），int型属性'time'表示该单项学习任务持续多少分钟。除了JSON数据的输出，不要包含任何其他文本。 " +
            "输出范围:我希望你能够以详细的任务执行内容来回答我，按照每一天具体到第几天、对应的任务项（包括任务名和具体任务内容）来安排日程，要求你的回答按照时间来排序。 ";
    private static final String PLANNER_PROMPT_FIT="能力与角色:我希望你是一位熟悉健身领域的专家 " +
            ...
            "输出范围:我希望你能够以详细的任务执行内容来回答我，按照每一天具体到第几天、对应的任务项（包括任务名和具体任务内容）来安排日程，要求你的回答按照时间来排序。 ";
    private static final String MESSAGES="{\"messages\":[{\"role\":\"user\",\"content\":\"request\"}]}";
    private static final String MESSAGES_STREAM="{\"messages\":[{\"role\":\"user\",\"content\":\"request\"}],\"stream\":true}";

    //无prompt，生成最普通的请求message(不推荐使用)
    public String getJsonMessage(String request){
        return MESSAGES.replace("request",request);
    }
    //无prompt stream(不推荐使用)
    public String getJsonMessageStream(String request){
        return MESSAGES_STREAM.replace("request",request);
    }
    //prompt型
    //普通计划类型message
